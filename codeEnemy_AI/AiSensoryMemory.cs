...
public class AiSensoryMemory
{
    ...
    public AiSensoryMemory (int maxPlayer)
    {
        character = new GameObject[maxPlayer];
    }
    public void UpdateSenses(AiSensor sensor)
    {
        int targets = sensor.Filter(character,"Character");
        for (int i =0; i < targets; ++i)
        {
            GameObject target = character[i];
    ...
    public void ForgetMemories(float olderThan)
    {
        ...
        memories.RemoveAll(m => m.gameObject.GetComponent<AIHealth>().Isdead());
    }
}
